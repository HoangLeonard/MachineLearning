...
# ╔═╡ ccffd468-ee31-11ed-3276-31d476fd489c
using MLDatasets

# ╔═╡ 197e914f-f4b1-43b7-8d63-4e65592fcab8
using ImageShow

# ╔═╡ 30684778-a854-4192-ba10-1cb9ebea52eb
using ImageIO

# ╔═╡ c47c113c-b644-445e-9480-518559e37ee5
using Flux

# ╔═╡ 354c0e1c-15fe-4f84-bfd3-86047281f626
dataset = MNIST(:train)

# ╔═╡ 9dc4907f-9ea1-4ae1-a022-c9c0a48d87ab
X = dataset.features[:,:,1:2000]

# ╔═╡ f01864da-c17a-4c80-923f-e5332ae877a8
x1 = X[:,:,1]

# ╔═╡ 5f66ca9d-faf5-4841-aa81-e5fcb708fb78
[convert2image(MNIST(), X[:,:,i]) for i=1:5]

# ╔═╡ c82e9566-ef2c-40f0-9024-bfbb17347852
y = dataset.targets[1:2000]

# ╔═╡ 1c952c43-054a-40a0-8e3d-6f6fed1a917b
y[1:5]

# ╔═╡ 523a72ef-7258-4ed2-a1a4-2c51d6aea61a
layer1 = Dense(5 => 3)

# ╔═╡ 569dde46-cd23-4610-8e24-ad6aa7810030
layer1.bias

# ╔═╡ bed4ded7-bc54-45d9-b0b2-cdb8a0224674
layer1.weight

# ╔═╡ 4678642e-e1aa-4484-81d3-012470056173
layer1.σ

# ╔═╡ 56c7e084-ddfe-4315-b93d-033681eabaf3
u = Float32.(rand(5))

# ╔═╡ 0de4d8d3-a9d9-4fb3-979e-94ec6e1cfc8b
layer1(u)

# ╔═╡ 57249562-2753-4548-8745-19c5f0a12f36
σ(layer1.weight * u + layer1.bias)

# ╔═╡ 83d6492b-2c45-4f26-8a61-4dee22300823
layer2 = Dense(5, 2, σ)

# ╔═╡ 2714a85d-6749-46f2-98a6-5e7e51535335
layer2(u)

# ╔═╡ b6007324-ceba-4cc1-b1da-998d0cc2e64c
layer2.σ(layer2.weight*u + layer2.bias)

# ╔═╡ b3b933ab-f654-46ab-b7dc-c4df237bc895
l1 = Dense(28*28, 64, tanh)	

# ╔═╡ 6b017a94-54cd-43a8-bf95-f11102950959
x1[:]

# ╔═╡ 375c9496-3de3-4478-b269-31d410be6b2c
a1 = l1(x1[:])

# ╔═╡ cefe1c28-9e9c-437a-89a0-4d89aec2e0c0
l2 = Dense(64 => 32, relu)

# ╔═╡ 2c49208a-538b-4da6-b9a2-fce7eae3f158
a2 = l2(a1)

# ╔═╡ 972ad55c-0c08-40e2-ba9d-2189d6552bad
network = Chain(l1, l2, Dense(32, 10), softmax)

# ╔═╡ fc97b83a-78cc-404e-bd8c-82eaca7fa59a
ŷ1 = network(x1[:])

# ╔═╡ 43823a22-ed02-4836-9144-9267a3c87876
y1 = y[1]

# ╔═╡ 365c147d-b9b4-47c6-b438-197030fb6771
Flux.onehot(y[1], 0:9)

# ╔═╡ 8f59ae6c-4e6a-4cc9-b9fb-16f4fbcdd7bd
loss(ŷ, y) = Flux.crossentropy(ŷ, y)

# ╔═╡ 084bf57d-9796-4e13-b2f7-8d3f5861b81f
loss(ŷ1, y1)

# ╔═╡ f5fd15a5-01fe-478e-808b-874e3108550f
X8 = zeros(Float32,28*28, 8)

# ╔═╡ 770023d2-db25-4f45-beed-243fba8cd4f8
for i=1:8
	X8[:,i] .= X[:, :, i][:] 
end

# ╔═╡ 8bcefdfb-59db-4e02-9022-c7770f7768b8
sum(dims=1, X8)

# ╔═╡ d422939c-e5dc-4189-ac37-4d82150182a5
ŷ8 = network(X8)

# ╔═╡ cfeb8356-56f3-495d-ba5c-fa5dc3211ade
y[1:8]

# ╔═╡ 2d3bc842-3b65-4c6b-9478-5d0952020fdb
y8 = Flux.onehotbatch(y[1:8], 0:9)  

# ╔═╡ 83f401c0-36ec-4704-9fe6-86c15c555413
loss(ŷ8, y8)

# ╔═╡ 25c8a4fc-b55b-4867-9e98-ffd3fb6fc11f
ŷ= [network(X[:,:,i][:]) for i=1:2000]

# ╔═╡ 72c63403-2630-4c5a-b957-bf1c6a20836a
prediction = Flux.stack(ŷ)

# ╔═╡ eb1c2cd8-5b78-4b2f-8033-0b8ab106cb7a
target = Flux.onehotbatch(y, 0:9)

# ╔═╡ d47b408d-1a5d-4024-9bc0-009cf5588f78
loss(prediction, target)

# ╔═╡ 7ab7729a-8996-492a-880d-b6b0365c49b5
sum(Flux.onecold(prediction) .== y) / length(y)

# ╔═╡ 293b4181-18a3-4213-8df3-b3e017852e25
X_train = [X[:,:,i][:] for i=1:2000]

# ╔═╡ fc8f752a-6ea9-43e6-b488-06bb3e286601
y_train = [Flux.onehot(y[i], 0:9) for i=1:2000]

# ╔═╡ 6fcc15da-9189-4c7e-9d9b-e4d9fe7c4c46
data = zip(X_train, y_train)

# ╔═╡ 433816b5-ff02-4f66-b091-03615d395b32
optimizer = Adam()

# ╔═╡ 30eb9c2c-010c-4da9-8c52-30189e67cd45
θ = Flux.params(network)

# ╔═╡ 5ff1eef7-01e6-4387-b23f-ecafe995a26c
opt_state = Flux.setup(optimizer, network)

# ╔═╡ fd148788-f530-4aed-8074-209bf504e721
cost(model, x, target) = Flux.crossentropy(model(x), target)

# ╔═╡ 30aa6579-ae0d-441e-ba7c-d42103760615
Flux.@epochs 10 Flux.train!(cost, network, data, opt_state)

# ╔═╡ d96f87ec-5799-47af-aa09-1a9b49ec0b66
z = [Flux.onecold(network(X_train[i])) for i=1:2000]

# ╔═╡ e9410b3b-bea4-47b1-9c16-d5fbcdfbcaec
sum(z .== (y .+ 1)) / length(y)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "34bc19beb11e917b08afd32be84a5237765d131e"
